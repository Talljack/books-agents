...
export function ModelSettings({ onClose }: { onClose?: () => void }) {
  ...
  const [selectedProvider, setSelectedProvider] = useState<string>(storedConfig.activeConfig.provider);
  const [selectedModel, setSelectedModel] = useState<string>(storedConfig.activeConfig.model);
  const [apiKey, setApiKey] = useState<string>("");
  const [customBaseUrl, setCustomBaseUrl] = useState<string>("");
  const [ollamaHost, setOllamaHost] = useState<string>(storedConfig.activeConfig.ollamaHost || "http://localhost:11434");
  const [customModel, setCustomModel] = useState<string>("");
  
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState<{ success: boolean; error?: string; latency?: number; models?: string[] } | null>(null);
  ...
  const currentProvider = getProviderConfig(selectedProvider as any);
  
  // 获取当前显示的模型列表
  const displayModels = dynamicModels.length > 0 ? dynamicModels : (currentProvider?.models || []);
  
  // 动态获取模型列表
  const fetchModels = useCallback(async (provider: string, key?: string, baseUrl?: string) => {
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">模型设置</h2>
      ...
      <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200">
        <div className="text-sm">
          ...
          <span className="text-blue-600 dark:text-blue-400">
            ...
            {getProviderConfig(storedConfig.activeConfig.provider)?.name} / {storedConfig.activeConfig.model}
          </span>
        </div>
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          {!currentProvider?.requiresApiKey && selectedProvider !== "ollama" && (
            <span className="text-xs text-green-600 bg-green-100 px-2 py-0.5 rounded">无需配置</span>
          )}
        </div>
        ...
        {selectedProvider === "custom" && (
          <div className="mt-3">
            <label className="block text-xs font-medium mb-1 text-gray-600">API 地址</label>
            <Input
              value={customBaseUrl}
      ...
      <div className="mb-6">
        ...
        {selectedProvider === "custom" ? (
          ...
          <>
            ...
            >
              {displayModels.map((model) => (
                <option key={model.id} value={model.id}>
                  {model.name} {model.isFree ? "🆓" : ""} {model.description ? `- ${model.description}` : ""}
                </option>
              ))}
            </select>
            ...
            {/* 模型列表状态提示 */}
            {loadingModels && (
              <p className="text-xs text-blue-600 mt-1">⏳ 正在获取模型列表...</p>
            )}
            {modelsError && (
              <p className="text-xs text-amber-600 mt-1">⚠️ {modelsError}</p>
            )}
            {hasApiModels && !modelsError && !loadingModels && (
              <p className="text-xs text-green-600 mt-1">✓ 已从 API 获取 {dynamicModels.length} 个可用模型</p>
            )}
            {!hasApiModels && !modelsError && !loadingModels && (
